...
# 설정(settings) - 에디터 (Editor) - 라이브 템플릿(livetempates) - python(사용구역)
# +- 로

# Abbreviation (약어) : 단축키로 저장할 단어
# Description (설명글) : 템플릿에 대한 설명글
# Template text : 단축키 실행했을 때 출력할 코드
# Define (정의) : 이 IDE에서 이 템플릿을 사용할 수 있는 범위 지정

# Lorem Ipsum : 의미없는 문자열의 나열, 공간채우기용 문자열


class Temp:
    def __init__(self, data):
        self.data = data
# 같은 이름의 변수 위치에는 같은 데이터가 자동으로 입력된다.

# 변수는 $$를 감싸서 지정할 수 있다.
# ex) $VAR$
# 변수는 템플릿을 사용했을 때 사용자가 직접 채워넣을 공간을 정하는 것

# $END$ : 템플릿에 모든 입력을 마쳤을 때 마우스 커서가 위치할 지점을 예약한다.

# 날짜 : 오전 11 : 31
# 이름 :

# 변수설정에 위치한 Expression(표현식)을 이용하면 변수 위치에 파이썬에 존재하는 내장 함수들을 연결할 수 있다

# date() : 오늘의 날짜가 입력된다.
# time() : 현재 시간이 입력된다.
# user() : 현재 컴퓨터의 사용자 이름이 입력된다
